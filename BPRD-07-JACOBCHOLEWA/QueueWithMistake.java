...
class QueueWithMistake {
    ...
    private static void runThreads(final int threads, final Queue queue) {
        ...
        System.out.printf("%-20s\t%4d\t%7.2f\t%s%n", queue.getClass().getName(), threads, timer.Check(), queue.get());
    }
}
...
class SentinelLockQueue implements Queue {  
    ...
    public synchronized int get() {
        ...
        Node first = head;
        head = first.next;
        first.next = null; /* THIS WAS THE SOLUTION */

        /* if the next reference is not nulled then the garbage collection will not collect the old element as it will still be referenced. When we eliminate the reference the garbage collector can collect the element and the memory can be freed. */
